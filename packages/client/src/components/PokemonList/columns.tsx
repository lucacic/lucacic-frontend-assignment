import { Tag, Image, Spin } from 'antd';
import { getPokemonColor, ColorType } from '../../utils/pokemonsType';

const defaultImage = "data:image/png;base64,"
const hostImage = "https://pokeres.bastionbot.org/images/pokemon";

/**
 * 
 * @param keySearch 
 */
const columns = (keySearch: string | null) => {

    const renderNameColumn = (name: string) => <h2 dangerouslySetInnerHTML={createCodeHtml(name)} />

    const renderClassificationColumn = (classification: string) => <h3>{classification}</h3>

    const renderTypesColumn = (types: [string]) => (
        <>
            {types?.map((type: string) => {
                return (
                    <Tag color={getPokemonColor((type.toLocaleLowerCase() as ColorType))} key={type}>
                        {type.toUpperCase()}
                    </Tag>
                );
            })}
        </>
    );

    const renderImageColumn = (id: any) => (
        <Image
            width={100}
            height={100}
            placeholder={<Spin />}
            fallback={defaultImage}
            src={`${hostImage}/${Number(id)}.png`}
        />
    )

    const createCodeHtml = (text: string) => (
        keySearch
            ? { __html: text.replace(new RegExp(keySearch, 'gi'), `<mark style="background: yellow;padding: 0; text-transform: capitalize;">${keySearch}</mark>`) }
            : { __html: text }

    )

    return (
        [
            {
                title: "",
                dataIndex: "id",
                key: "id",
                render: renderImageColumn
            },
            {
                title: 'Name',
                dataIndex: 'name',
                key: 'name',
                render: renderNameColumn
            },
            {
                title: 'Classification',
                dataIndex: 'classification',
                key: 'classification',
                render: renderClassificationColumn
            },
            {
                title: 'Types',
                key: 'types',
                dataIndex: 'types',
                render: renderTypesColumn
            },
        ]
    )

}


export default columns;